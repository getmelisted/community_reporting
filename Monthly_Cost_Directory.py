...
def SendEmail(attachment):
    ...
    postmark.emails.send(
                         ...
                         HtmlBody= 'Please find attached the cost associated to Directories for the previous month.',
...
def getnbwo(workorders, dirid, dirname):
    ...
    for wo in workorders:
        if wo[1] == dirid and wo[7] != 0 and wo[7] != 8 and wo[7] != 62 and wo[7] != 63:
            filteredwos = filteredwos + wo[5]
            sum = sum + wo[10]
            
    total = [dirid, dirname, filteredwos, sum]
