...
def writeRAWCSV(workorder, file):
    ...
    with open(file, 'w', newline='', encoding='ISO-8859-1') as csvfile:
        ...
        csv_writer.writerow(["wo_id", "wo_date", "wo_type", "elapsed_time", "updatedat", "user_name", "user_trust_level", "old_status", "new_status"])
        
        for wo in workorder:
            csv_writer.writerow([wo[0], wo[1], wo[2], wo[3], wo[4], wo[5], wo[6], wo[7], wo[8]])
...
def SendEmail(attachments):
    ...
    postmark.emails.send(
                         ...
                         Subject='Weekly Facebook Review Response Community Results',
                         HtmlBody= 'Please find attached the Facebook Review Response Community Results associated to Work Orders for the previous week.',
...
def main():
    ...
    worocompleted = ["Work Orders to Completed (Audited)", getnewstatnb(workorders, 12)]
    ...
    pertocompleted = ["% to Completed (Audited)", getpercentage(totalwoattempt[1], worocompleted[1])]
    ...
    file = "./Archive/WeeklyFacebookCommunityReviewResponse - " + str(sunday) + " to " + str(saturday) + ".csv"

    arrays =[totalwoattempt, worocompleted, wotosubpending, wotofailed, pertocompleted, pertosubpending, pertofailed, averageage]
    writeCSV(arrays, file)
    
    file2 = "./Archive/RAW_WeeklyFacebookCommunityReviewResponse - " + str(sunday) + " to " + str(saturday) + ".csv"
